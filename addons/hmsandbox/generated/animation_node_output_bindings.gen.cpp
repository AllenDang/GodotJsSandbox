...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AnimationNodeOutput: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AnimationNodeOutput: unknown error"); \
    }


// Registration function for AnimationNodeOutput
void register_AnimationNodeOutput_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AnimationNodeOutput", class_info);
}
