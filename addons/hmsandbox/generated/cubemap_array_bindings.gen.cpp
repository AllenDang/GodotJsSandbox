...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "CubemapArray: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "CubemapArray: unknown error"); \
    }

// Method: CubemapArray::create_placeholder
static JSValue js_CubemapArray_create_placeholder(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "CubemapArray.create_placeholder: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "CubemapArray.create_placeholder: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::HEAVY)) {
        return JS_ThrowInternalError(ctx, "CubemapArray.create_placeholder: heavy rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "CubemapArray.create_placeholder: invalid or freed object");
    }

    CubemapArray* typed_obj = Object::cast_to<CubemapArray>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "CubemapArray.create_placeholder: object is not a CubemapArray");
    }
...
// Registration function for CubemapArray
void register_CubemapArray_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, methods, "create_placeholder",
        JS_NewCFunction(ctx, js_CubemapArray_create_placeholder, "create_placeholder", 1));
    ...
    JS_SetPropertyStr(ctx, classes, "CubemapArray", class_info);
}
