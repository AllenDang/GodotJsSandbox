...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioBusLayout: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioBusLayout: unknown error"); \
    }


// Registration function for AudioBusLayout
void register_AudioBusLayout_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioBusLayout", class_info);
}
