...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "FoldableGroup: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "FoldableGroup: unknown error"); \
    }

// Method: FoldableGroup::get_expanded_container
static JSValue js_FoldableGroup_get_expanded_container(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.get_expanded_container: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.get_expanded_container: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.get_expanded_container: invalid or freed object");
    }

    FoldableGroup* typed_obj = Object::cast_to<FoldableGroup>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.get_expanded_container: object is not a FoldableGroup");
    }

    FoldableContainer* result = typed_obj->get_expanded_container();
...
static JSValue js_FoldableGroup_get_allow_folding_all(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all getter: missing handle");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all getter: invalid handle");
    }
    ...
    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all getter: invalid object");
    }

    FoldableGroup* typed_obj = Object::cast_to<FoldableGroup>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all getter: wrong type");
    }

    bool value = typed_obj->is_allow_folding_all();
    return JS_NewBool(ctx, value);
}

// Property setter: FoldableGroup::allow_folding_all
static JSValue js_FoldableGroup_set_allow_folding_all(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all setter: missing arguments");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all setter: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "FoldableGroup.allow_folding_all setter: write rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all setter: invalid object");
    }

    FoldableGroup* typed_obj = Object::cast_to<FoldableGroup>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "FoldableGroup.allow_folding_all setter: wrong type");
    }

    bool value = JS_ToBool(ctx, argv[1]);
    typed_obj->set_allow_folding_all(value);
    return JS_UNDEFINED;
}


// Registration function for FoldableGroup
void register_FoldableGroup_bindings(JSContext* ctx, JSValue global, JSValue classes) {
...

    {
        ...
        JS_SetPropertyStr(ctx, prop_obj, "get",
            JS_NewCFunction(ctx, js_FoldableGroup_get_allow_folding_all, "get_allow_folding_all", 1));
        JS_SetPropertyStr(ctx, prop_obj, "set",
            JS_NewCFunction(ctx, js_FoldableGroup_set_allow_folding_all, "set_allow_folding_all", 2));
        JS_SetPropertyStr(ctx, props, "allow_folding_all", prop_obj);
    }

    // Register signals array
    JSValue signals_arr = JS_NewArray(ctx);
    JS_SetPropertyUint32(ctx, signals_arr, 0, JS_NewString(ctx, "expanded"));
    ...
    JS_SetPropertyStr(ctx, classes, "FoldableGroup", class_info);
}
