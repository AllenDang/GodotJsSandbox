...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "QuadMesh: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "QuadMesh: unknown error"); \
    }


// Registration function for QuadMesh
void register_QuadMesh_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "PlaneMesh"));
    ...
    JS_SetPropertyStr(ctx, classes, "QuadMesh", class_info);
}
