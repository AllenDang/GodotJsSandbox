...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Compositor: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Compositor: unknown error"); \
    }


// Registration function for Compositor
void register_Compositor_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Compositor", class_info);
}
