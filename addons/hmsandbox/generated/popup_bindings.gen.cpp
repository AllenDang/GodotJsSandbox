...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Popup: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Popup: unknown error"); \
    }


// Registration function for Popup
void register_Popup_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyUint32(ctx, signals_arr, 0, JS_NewString(ctx, "popup_hide"));
    ...
    JS_SetPropertyStr(ctx, classes, "Popup", class_info);
}
