...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "WebRTCDataChannelExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "WebRTCDataChannelExtension: unknown error"); \
    }


// Registration function for WebRTCDataChannelExtension
void register_WebRTCDataChannelExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "WebRTCDataChannel"));
    ...
    JS_SetPropertyStr(ctx, classes, "WebRTCDataChannelExtension", class_info);
}
