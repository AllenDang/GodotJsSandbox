...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Cubemap: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Cubemap: unknown error"); \
    }

// Method: Cubemap::create_placeholder
static JSValue js_Cubemap_create_placeholder(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "Cubemap.create_placeholder: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "Cubemap.create_placeholder: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::HEAVY)) {
        return JS_ThrowInternalError(ctx, "Cubemap.create_placeholder: heavy rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "Cubemap.create_placeholder: invalid or freed object");
    }

    Cubemap* typed_obj = Object::cast_to<Cubemap>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "Cubemap.create_placeholder: object is not a Cubemap");
    }
...
// Registration function for Cubemap
void register_Cubemap_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, methods, "create_placeholder",
        JS_NewCFunction(ctx, js_Cubemap_create_placeholder, "create_placeholder", 1));
    ...
    JS_SetPropertyStr(ctx, classes, "Cubemap", class_info);
}
