...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "TextServerExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "TextServerExtension: unknown error"); \
    }


// Registration function for TextServerExtension
void register_TextServerExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "TextServer"));
    ...
    JS_SetPropertyStr(ctx, classes, "TextServerExtension", class_info);
}
