...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VSlider: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VSlider: unknown error"); \
    }


// Registration function for VSlider
void register_VSlider_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "VSlider", class_info);
}
