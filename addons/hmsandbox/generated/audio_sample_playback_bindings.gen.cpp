...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioSamplePlayback: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioSamplePlayback: unknown error"); \
    }


// Registration function for AudioSamplePlayback
void register_AudioSamplePlayback_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioSamplePlayback", class_info);
}
