...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectHighPassFilter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectHighPassFilter: unknown error"); \
    }


// Registration function for AudioEffectHighPassFilter
void register_AudioEffectHighPassFilter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectHighPassFilter", class_info);
}
