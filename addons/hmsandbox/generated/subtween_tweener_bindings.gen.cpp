...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "SubtweenTweener: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "SubtweenTweener: unknown error"); \
    }

// Method: SubtweenTweener::set_delay
static JSValue js_SubtweenTweener_set_delay(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "SubtweenTweener.set_delay: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "SubtweenTweener.set_delay: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "SubtweenTweener.set_delay: write rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "SubtweenTweener.set_delay: invalid or freed object");
    }

    SubtweenTweener* typed_obj = Object::cast_to<SubtweenTweener>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "SubtweenTweener.set_delay: object is not a SubtweenTweener");
    }

    // Argument count check (excluding handle) - only required args
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "SubtweenTweener.set_delay: expected at least 1 arguments, got %d", argc - 1);
    }

    // Convert arguments
    double arg_delay; JS_ToFloat64(ctx, &arg_delay, argv[1]);

    Ref<SubtweenTweener> result = typed_obj->set_delay(arg_delay);
...
// Registration function for SubtweenTweener
void register_SubtweenTweener_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, methods, "set_delay",
        JS_NewCFunction(ctx, js_SubtweenTweener_set_delay, "set_delay", 2));
    ...
    JS_SetPropertyStr(ctx, classes, "SubtweenTweener", class_info);
}
