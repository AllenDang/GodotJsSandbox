...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "HScrollBar: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "HScrollBar: unknown error"); \
    }


// Registration function for HScrollBar
void register_HScrollBar_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "ScrollBar"));
    ...
    JS_SetPropertyStr(ctx, classes, "HScrollBar", class_info);
}
