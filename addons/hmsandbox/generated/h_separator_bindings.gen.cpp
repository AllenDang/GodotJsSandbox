...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "HSeparator: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "HSeparator: unknown error"); \
    }


// Registration function for HSeparator
void register_HSeparator_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "Separator"));
    ...
    JS_SetPropertyStr(ctx, classes, "HSeparator", class_info);
}
