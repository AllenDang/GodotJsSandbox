...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PanelContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "PanelContainer: unknown error"); \
    }


// Registration function for PanelContainer
void register_PanelContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "PanelContainer", class_info);
}
