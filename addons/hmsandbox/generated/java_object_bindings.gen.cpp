...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "JavaObject: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "JavaObject: unknown error"); \
    }

// Method: JavaObject::get_java_class
static JSValue js_JavaObject_get_java_class(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "JavaObject.get_java_class: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "JavaObject.get_java_class: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "JavaObject.get_java_class: invalid or freed object");
    }

    JavaObject* typed_obj = Object::cast_to<JavaObject>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "JavaObject.get_java_class: object is not a JavaObject");
    }

    Ref<JavaClass> result = typed_obj->get_java_class();
...
// Registration function for JavaObject
void register_JavaObject_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, methods, "get_java_class",
        JS_NewCFunction(ctx, js_JavaObject_get_java_class, "get_java_class", 1));
    ...
    JS_SetPropertyStr(ctx, classes, "JavaObject", class_info);
}
