...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "HFlowContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "HFlowContainer: unknown error"); \
    }


// Registration function for HFlowContainer
void register_HFlowContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "FlowContainer"));
    ...
    JS_SetPropertyStr(ctx, classes, "HFlowContainer", class_info);
}
