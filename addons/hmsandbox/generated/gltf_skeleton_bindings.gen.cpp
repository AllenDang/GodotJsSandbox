...
static JSValue js_GLTFSkeleton_set_joints(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "GLTFSkeleton.joints setter: write rate limit exceeded");
    }
...
static JSValue js_GLTFSkeleton_set_roots(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    ExecutionLimiter* limiter = qjs_ctx->get_execution_limiter();
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "GLTFSkeleton.roots setter: write rate limit exceeded");
    }
...
static JSValue js_GLTFSkeleton_set_godot_bone_node(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    ExecutionLimiter* limiter = qjs_ctx->get_execution_limiter();
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "GLTFSkeleton.godot_bone_node setter: write rate limit exceeded");
    }
