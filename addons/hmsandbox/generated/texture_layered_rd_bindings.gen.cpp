...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "TextureLayeredRD: %s", e.what()); \
    } catch (...) { \
...
static JSValue js_TextureLayeredRD_set_texture_rd_rid(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "TextureLayeredRD.texture_rd_rid setter: write rate limit exceeded");
    }
...
void register_TextureLayeredRD_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "TextureLayeredRD", class_info);
}
