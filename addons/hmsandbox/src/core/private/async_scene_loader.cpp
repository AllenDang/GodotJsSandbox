    ...
    switch (stage_) {
        ...
        case STAGE_ATTACHING: {
            ...
            if (sandbox_) {
                ...
                String scene_name = loaded_scene_->get_name();
            }
...
void AsyncSceneLoader::collect_script_paths(Node* node) {
    ...
    // Check if this node has a script that could be a JSScript
    Ref<Script> script = node->get_script();
    if (script.is_valid()) {
        // IMPORTANT: In Godot 4.6, threaded resource loading may create placeholder
        // Script objects that report as "JSScript" type but have corrupted vtables.
        // We must NOT call any JSScript-specific virtual methods here.
        //
        // Instead, use Resource::get_path() which is inherited and safe to call.
        // For .js files, we can identify them by path extension.
        String path = script->get_path();
        if (!path.is_empty() && path.get_extension().to_lower() == "js") {
            if (script_paths_.find(path) < 0) {
...
void AsyncSceneLoader::reattach_scripts_with_content(Node* node) {
    ...
    // Check if this node has a script that needs reattachment
    Ref<Script> current_script = node->get_script();
    
    if (current_script.is_valid()) {
        // IMPORTANT: In Godot 4.6, threaded resource loading may create placeholder
        // Script objects with corrupted vtables. Use safe methods only.
        // Resource::get_path() is inherited from Resource and safe to call.
        String script_path = current_script->get_path();

        // Only process .js files
        if (!script_path.is_empty() && script_path.get_extension().to_lower() == "js") {
            ...
            if (path_index >= 0 && path_index < script_contents_.size()) {
                ...
                if (!source_code.is_empty()) {
                    ...
                    // This creates a REAL JSScript with proper vtable
                    ...
                    if (sandbox_script.is_valid()) {
                        ...
                        // Replace the potentially corrupted script with a proper one
                        node->set_script(sandbox_script);
                        UtilityFunctions::print(node->get_name());
                        UtilityFunctions::print(new_js_script->get_path());
                    }
                }
            }
        }
