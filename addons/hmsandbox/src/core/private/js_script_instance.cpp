...
JSScriptInstance::~JSScriptInstance() {
    ...
    JSScript* script = get_script();
    if (script) {
        script->unregister_instance(owner_);
    }
    // script_ref_ is automatically released when this instance is destroyed
}

void JSScriptInstance::set_script(JSScript* p_script) {
    // Store as Ref<Script> to keep the script alive via reference counting
    // This prevents use-after-free when the script's other references are released
    script_ref_ = Ref<Script>(p_script);
}

JSScript* JSScriptInstance::get_script() const {
    return Object::cast_to<JSScript>(script_ref_.ptr());
}
...
bool JSScriptInstance::initialize() {
    ...
    String error;
    js_instance_id_ = ctx->create_script_instance(source, script->get_path(), owner_, error);

    if (js_instance_id_ == 0) {
        UtilityFunctions::printerr("Failed to create JS instance: ", error);
        // Route error to sandbox for AI feedback
        JSSandbox* sandbox = script->get_sandbox();
        if (sandbox) {
            String file_path = script->get_path();
...
GDExtensionObjectPtr JSScriptInstance::_get_script(GDExtensionScriptInstanceDataPtr p_instance) {
    JSScriptInstance* self = (JSScriptInstance*)p_instance;
    JSScript* script = self->get_script();
    if (script) {
...
GDExtensionScriptInstancePtr JSScriptInstance::create_instance(JSScriptInstance* p_instance) {
    ...
    if (!script_instance_create3) {
        script_instance_create3 = (GDExtensionInterfaceScriptInstanceCreate3)
            gdextension_interface::get_proc_address("script_instance_create3");
    }
