...
public:
    ...
    // Returns the JSScript pointer (may be null)
    // The script is kept alive by script_ref_ as long as this instance exists
    JSScript* get_script() const;

    // Returns the Ref<Script> for proper reference counting when needed externally
    godot::Ref<godot::Script> get_script_ref() const { return script_ref_; }
...
private:
    ...
    // Reference-counted script storage - keeps JSScript alive as long as this instance exists
    // This fixes use-after-free when the script's other references are released
    godot::Ref<godot::Script> script_ref_;
