...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "RenderSceneDataExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "RenderSceneDataExtension: unknown error"); \
    }


// Registration function for RenderSceneDataExtension
void register_RenderSceneDataExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "RenderSceneData"));
    ...
    JS_SetPropertyStr(ctx, classes, "RenderSceneDataExtension", class_info);
}
