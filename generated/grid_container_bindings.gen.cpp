...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "GridContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "GridContainer: unknown error"); \
    }

// Property getter: GridContainer::columns
static JSValue js_GridContainer_get_columns(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns getter: missing handle");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns getter: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns getter: invalid object");
    }

    GridContainer* typed_obj = Object::cast_to<GridContainer>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns getter: wrong type");
    }
...
// Property setter: GridContainer::columns
static JSValue js_GridContainer_set_columns(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns setter: missing arguments");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns setter: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "GridContainer.columns setter: write rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns setter: invalid object");
    }

    GridContainer* typed_obj = Object::cast_to<GridContainer>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "GridContainer.columns setter: wrong type");
    }
...
// Registration function for GridContainer
void register_GridContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
...

    {
        ...
        JS_SetPropertyStr(ctx, prop_obj, "get",
            JS_NewCFunction(ctx, js_GridContainer_get_columns, "get_columns", 1));
        JS_SetPropertyStr(ctx, prop_obj, "set",
            JS_NewCFunction(ctx, js_GridContainer_set_columns, "set_columns", 2));
    ...
    JS_SetPropertyStr(ctx, classes, "GridContainer", class_info);
}
