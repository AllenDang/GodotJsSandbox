...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "LightmapperRD: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "LightmapperRD: unknown error"); \
    }


// Registration function for LightmapperRD
void register_LightmapperRD_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "Lightmapper"));
    ...
    JS_SetPropertyStr(ctx, classes, "LightmapperRD", class_info);
}
