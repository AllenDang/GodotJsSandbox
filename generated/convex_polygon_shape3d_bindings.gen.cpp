...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ConvexPolygonShape3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ConvexPolygonShape3D: unknown error"); \
    }


// Registration function for ConvexPolygonShape3D
void register_ConvexPolygonShape3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "ConvexPolygonShape3D", class_info);
}
