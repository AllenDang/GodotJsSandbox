...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Separator: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Separator: unknown error"); \
    }


// Registration function for Separator
void register_Separator_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Separator", class_info);
}
