...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PhysicsDirectBodyState2DExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "PhysicsDirectBodyState2DExtension: unknown error"); \
    }


// Registration function for PhysicsDirectBodyState2DExtension
void register_PhysicsDirectBodyState2DExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "PhysicsDirectBodyState2D"));
    ...
    JS_SetPropertyStr(ctx, classes, "PhysicsDirectBodyState2DExtension", class_info);
}
