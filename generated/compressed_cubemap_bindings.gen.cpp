...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "CompressedCubemap: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "CompressedCubemap: unknown error"); \
    }


// Registration function for CompressedCubemap
void register_CompressedCubemap_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "CompressedCubemap", class_info);
}
