...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioStreamPlaybackPlaylist: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioStreamPlaybackPlaylist: unknown error"); \
    }


// Registration function for AudioStreamPlaybackPlaylist
void register_AudioStreamPlaybackPlaylist_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioStreamPlaybackPlaylist", class_info);
}
