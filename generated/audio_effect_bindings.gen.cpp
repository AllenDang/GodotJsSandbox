...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffect: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffect: unknown error"); \
    }


// Registration function for AudioEffect
void register_AudioEffect_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffect", class_info);
}
