...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "SpotLight3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "SpotLight3D: unknown error"); \
    }


// Registration function for SpotLight3D
void register_SpotLight3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "SpotLight3D", class_info);
}
