...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "StreamPeerExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "StreamPeerExtension: unknown error"); \
    }


// Registration function for StreamPeerExtension
void register_StreamPeerExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "StreamPeerExtension", class_info);
}
