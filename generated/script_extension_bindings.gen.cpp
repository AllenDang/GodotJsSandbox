...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ScriptExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ScriptExtension: unknown error"); \
    }


// Registration function for ScriptExtension
void register_ScriptExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "Script"));
    ...
    JS_SetPropertyStr(ctx, classes, "ScriptExtension", class_info);
}
