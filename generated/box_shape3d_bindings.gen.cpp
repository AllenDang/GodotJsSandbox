...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "BoxShape3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "BoxShape3D: unknown error"); \
    }

// Property getter: BoxShape3D::size
static JSValue js_BoxShape3D_get_size(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size getter: missing handle");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size getter: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size getter: invalid object");
    }

    BoxShape3D* typed_obj = Object::cast_to<BoxShape3D>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size getter: wrong type");
    }
...
// Property setter: BoxShape3D::size
static JSValue js_BoxShape3D_set_size(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size setter: missing arguments");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size setter: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "BoxShape3D.size setter: write rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size setter: invalid object");
    }

    BoxShape3D* typed_obj = Object::cast_to<BoxShape3D>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "BoxShape3D.size setter: wrong type");
    }
...
// Registration function for BoxShape3D
void register_BoxShape3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
...

    {
        ...
        JS_SetPropertyStr(ctx, prop_obj, "get",
            JS_NewCFunction(ctx, js_BoxShape3D_get_size, "get_size", 1));
        JS_SetPropertyStr(ctx, prop_obj, "set",
            JS_NewCFunction(ctx, js_BoxShape3D_set_size, "set_size", 2));
    ...
    JS_SetPropertyStr(ctx, classes, "BoxShape3D", class_info);
}
