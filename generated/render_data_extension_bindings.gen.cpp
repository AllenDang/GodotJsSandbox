...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "RenderDataExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "RenderDataExtension: unknown error"); \
    }


// Registration function for RenderDataExtension
void register_RenderDataExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "RenderData"));
    ...
    JS_SetPropertyStr(ctx, classes, "RenderDataExtension", class_info);
}
