...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "OpenXRExtensionWrapperExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "OpenXRExtensionWrapperExtension: unknown error"); \
    }


// Registration function for OpenXRExtensionWrapperExtension
void register_OpenXRExtensionWrapperExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "OpenXRExtensionWrapperExtension", class_info);
}
