...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "JavaClass: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "JavaClass: unknown error"); \
    }

// Method: JavaClass::get_java_class_name
static JSValue js_JavaClass_get_java_class_name(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_class_name: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_class_name: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_class_name: invalid or freed object");
    }

    JavaClass* typed_obj = Object::cast_to<JavaClass>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_class_name: object is not a JavaClass");
    }

    String result = typed_obj->get_java_class_name();
...
static JSValue js_JavaClass_get_java_parent_class(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_parent_class: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_parent_class: invalid handle");
    }
    ...
    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_parent_class: invalid or freed object");
    }

    JavaClass* typed_obj = Object::cast_to<JavaClass>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "JavaClass.get_java_parent_class: object is not a JavaClass");
    }

    Ref<JavaClass> result = typed_obj->get_java_parent_class();
...
// Registration function for JavaClass
void register_JavaClass_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "JavaClass", class_info);
}
