...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ShaderGlobalsOverride: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ShaderGlobalsOverride: unknown error"); \
    }


// Registration function for ShaderGlobalsOverride
void register_ShaderGlobalsOverride_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "ShaderGlobalsOverride", class_info);
}
