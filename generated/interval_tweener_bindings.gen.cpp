...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "IntervalTweener: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "IntervalTweener: unknown error"); \
    }


// Registration function for IntervalTweener
void register_IntervalTweener_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "IntervalTweener", class_info);
}
