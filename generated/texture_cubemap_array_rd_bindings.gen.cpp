...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "TextureCubemapArrayRD: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "TextureCubemapArrayRD: unknown error"); \
    }


// Registration function for TextureCubemapArrayRD
void register_TextureCubemapArrayRD_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "TextureLayeredRD"));
    ...
    JS_SetPropertyStr(ctx, classes, "TextureCubemapArrayRD", class_info);
}
