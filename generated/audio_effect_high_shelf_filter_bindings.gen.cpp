...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectHighShelfFilter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectHighShelfFilter: unknown error"); \
    }


// Registration function for AudioEffectHighShelfFilter
void register_AudioEffectHighShelfFilter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "AudioEffectFilter"));
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectHighShelfFilter", class_info);
}
