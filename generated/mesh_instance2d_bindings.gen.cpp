...
static JSValue js_MeshInstance2D_set_mesh(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "MeshInstance2D.mesh setter: write rate limit exceeded");
    }
...
static JSValue js_MeshInstance2D_set_texture(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    ExecutionLimiter* limiter = qjs_ctx->get_execution_limiter();
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "MeshInstance2D.texture setter: write rate limit exceeded");
    }
