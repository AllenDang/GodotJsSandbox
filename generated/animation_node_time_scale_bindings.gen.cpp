...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AnimationNodeTimeScale: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AnimationNodeTimeScale: unknown error"); \
    }


// Registration function for AnimationNodeTimeScale
void register_AnimationNodeTimeScale_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AnimationNodeTimeScale", class_info);
}
