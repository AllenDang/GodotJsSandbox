...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "GLTFDocumentExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "GLTFDocumentExtension: unknown error"); \
    }


// Registration function for GLTFDocumentExtension
void register_GLTFDocumentExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "GLTFDocumentExtension", class_info);
}
