...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "HSlider: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "HSlider: unknown error"); \
    }


// Registration function for HSlider
void register_HSlider_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "Slider"));
    ...
    JS_SetPropertyStr(ctx, classes, "HSlider", class_info);
}
