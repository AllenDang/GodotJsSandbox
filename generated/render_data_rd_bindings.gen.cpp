...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "RenderDataRD: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "RenderDataRD: unknown error"); \
    }


// Registration function for RenderDataRD
void register_RenderDataRD_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "RenderDataRD", class_info);
}
