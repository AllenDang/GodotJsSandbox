...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "OpenXRActionBindingModifier: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "OpenXRActionBindingModifier: unknown error"); \
    }


// Registration function for OpenXRActionBindingModifier
void register_OpenXRActionBindingModifier_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "OpenXRBindingModifier"));
    ...
    JS_SetPropertyStr(ctx, classes, "OpenXRActionBindingModifier", class_info);
}
