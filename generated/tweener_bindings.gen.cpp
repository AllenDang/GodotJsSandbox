...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Tweener: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Tweener: unknown error"); \
    }


// Registration function for Tweener
void register_Tweener_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Tweener", class_info);
}
