...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PopupPanel: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "PopupPanel: unknown error"); \
    }


// Registration function for PopupPanel
void register_PopupPanel_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "PopupPanel", class_info);
}
