...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectLowShelfFilter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectLowShelfFilter: unknown error"); \
    }


// Registration function for AudioEffectLowShelfFilter
void register_AudioEffectLowShelfFilter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectLowShelfFilter", class_info);
}
