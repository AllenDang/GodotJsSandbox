...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ResourceFormatLoader: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ResourceFormatLoader: unknown error"); \
    }


// Registration function for ResourceFormatLoader
void register_ResourceFormatLoader_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "ResourceFormatLoader", class_info);
}
