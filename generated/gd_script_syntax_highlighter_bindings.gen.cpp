...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "GDScriptSyntaxHighlighter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "GDScriptSyntaxHighlighter: unknown error"); \
    }


// Registration function for GDScriptSyntaxHighlighter
void register_GDScriptSyntaxHighlighter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "EditorSyntaxHighlighter"));
    ...
    JS_SetPropertyStr(ctx, classes, "GDScriptSyntaxHighlighter", class_info);
}
