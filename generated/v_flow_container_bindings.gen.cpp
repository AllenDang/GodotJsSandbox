...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VFlowContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VFlowContainer: unknown error"); \
    }


// Registration function for VFlowContainer
void register_VFlowContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "VFlowContainer", class_info);
}
