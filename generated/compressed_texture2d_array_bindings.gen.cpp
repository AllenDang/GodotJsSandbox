...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "CompressedTexture2DArray: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "CompressedTexture2DArray: unknown error"); \
    }


// Registration function for CompressedTexture2DArray
void register_CompressedTexture2DArray_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "CompressedTextureLayered"));
    ...
    JS_SetPropertyStr(ctx, classes, "CompressedTexture2DArray", class_info);
}
