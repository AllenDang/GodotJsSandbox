...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "XRControllerTracker: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "XRControllerTracker: unknown error"); \
    }


// Registration function for XRControllerTracker
void register_XRControllerTracker_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "XRControllerTracker", class_info);
}
