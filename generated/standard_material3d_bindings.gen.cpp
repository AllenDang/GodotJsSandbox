...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "StandardMaterial3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "StandardMaterial3D: unknown error"); \
    }


// Registration function for StandardMaterial3D
void register_StandardMaterial3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "BaseMaterial3D"));
    ...
    JS_SetPropertyStr(ctx, classes, "StandardMaterial3D", class_info);
}
