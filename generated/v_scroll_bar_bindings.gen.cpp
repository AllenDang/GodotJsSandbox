...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VScrollBar: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VScrollBar: unknown error"); \
    }


// Registration function for VScrollBar
void register_VScrollBar_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "VScrollBar", class_info);
}
