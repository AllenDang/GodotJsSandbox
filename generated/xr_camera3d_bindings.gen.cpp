...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "XRCamera3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "XRCamera3D: unknown error"); \
    }


// Registration function for XRCamera3D
void register_XRCamera3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "Camera3D"));
    ...
    JS_SetPropertyStr(ctx, classes, "XRCamera3D", class_info);
}
