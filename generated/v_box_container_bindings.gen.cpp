...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VBoxContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VBoxContainer: unknown error"); \
    }


// Registration function for VBoxContainer
void register_VBoxContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "BoxContainer"));
    ...
    JS_SetPropertyStr(ctx, classes, "VBoxContainer", class_info);
}
