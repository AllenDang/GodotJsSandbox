...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "JavaScriptObject: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "JavaScriptObject: unknown error"); \
    }


// Registration function for JavaScriptObject
void register_JavaScriptObject_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "JavaScriptObject", class_info);
}
