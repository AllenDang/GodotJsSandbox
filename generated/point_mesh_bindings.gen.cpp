...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PointMesh: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "PointMesh: unknown error"); \
    }


// Registration function for PointMesh
void register_PointMesh_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "PointMesh", class_info);
}
