...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "MainLoop: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "MainLoop: unknown error"); \
    }


// Registration function for MainLoop
void register_MainLoop_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyUint32(ctx, signals_arr, 0, JS_NewString(ctx, "on_request_permissions_result"));
    ...
    JS_SetPropertyStr(ctx, classes, "MainLoop", class_info);
}
