...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ScriptLanguageExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ScriptLanguageExtension: unknown error"); \
    }


// Registration function for ScriptLanguageExtension
void register_ScriptLanguageExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "ScriptLanguage"));
    ...
    JS_SetPropertyStr(ctx, classes, "ScriptLanguageExtension", class_info);
}
