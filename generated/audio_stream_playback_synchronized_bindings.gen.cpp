...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioStreamPlaybackSynchronized: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioStreamPlaybackSynchronized: unknown error"); \
    }


// Registration function for AudioStreamPlaybackSynchronized
void register_AudioStreamPlaybackSynchronized_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioStreamPlaybackSynchronized", class_info);
}
