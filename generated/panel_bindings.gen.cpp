...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Panel: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Panel: unknown error"); \
    }


// Registration function for Panel
void register_Panel_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Panel", class_info);
}
