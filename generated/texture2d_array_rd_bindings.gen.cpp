...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Texture2DArrayRD: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Texture2DArrayRD: unknown error"); \
    }


// Registration function for Texture2DArrayRD
void register_Texture2DArrayRD_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Texture2DArrayRD", class_info);
}
