...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ConeTwistJoint3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ConeTwistJoint3D: unknown error"); \
    }


// Registration function for ConeTwistJoint3D
void register_ConeTwistJoint3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "ConeTwistJoint3D", class_info);
}
