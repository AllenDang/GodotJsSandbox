...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PacketPeerExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "PacketPeerExtension: unknown error"); \
    }


// Registration function for PacketPeerExtension
void register_PacketPeerExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "PacketPeerExtension", class_info);
}
