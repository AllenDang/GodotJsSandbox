...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Logger: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Logger: unknown error"); \
    }


// Registration function for Logger
void register_Logger_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Logger", class_info);
}
