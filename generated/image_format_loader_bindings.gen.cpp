...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ImageFormatLoader: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ImageFormatLoader: unknown error"); \
    }


// Registration function for ImageFormatLoader
void register_ImageFormatLoader_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "ImageFormatLoader", class_info);
}
