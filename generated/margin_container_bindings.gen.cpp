...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "MarginContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "MarginContainer: unknown error"); \
    }


// Registration function for MarginContainer
void register_MarginContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "MarginContainer", class_info);
}
