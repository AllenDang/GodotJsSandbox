...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "World2D: %s", e.what()); \
    } catch (...) { \
...
static JSValue js_World2D_get_direct_space_state(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "World2D.direct_space_state getter: missing handle");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "World2D.direct_space_state getter: invalid handle");
    }
    ...
    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "World2D.direct_space_state getter: invalid object");
    }

    World2D* typed_obj = Object::cast_to<World2D>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "World2D.direct_space_state getter: wrong type");
    }

    PhysicsDirectSpaceState2D* value = typed_obj->get_direct_space_state();
...
// Registration function for World2D
void register_World2D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    }
    {
        JSValue prop_obj = JS_NewObject(ctx);
        JS_SetPropertyStr(ctx, prop_obj, "get",
            JS_NewCFunction(ctx, js_World2D_get_direct_space_state, "get_direct_space_state", 1));
    ...
    JS_SetPropertyStr(ctx, classes, "World2D", class_info);
}
