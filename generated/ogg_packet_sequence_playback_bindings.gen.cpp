...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "OggPacketSequencePlayback: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "OggPacketSequencePlayback: unknown error"); \
    }


// Registration function for OggPacketSequencePlayback
void register_OggPacketSequencePlayback_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "OggPacketSequencePlayback", class_info);
}
