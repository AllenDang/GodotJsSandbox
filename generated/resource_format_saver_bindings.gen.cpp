...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "ResourceFormatSaver: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "ResourceFormatSaver: unknown error"); \
    }


// Registration function for ResourceFormatSaver
void register_ResourceFormatSaver_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "ResourceFormatSaver", class_info);
}
