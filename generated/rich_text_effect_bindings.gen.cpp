...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "RichTextEffect: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "RichTextEffect: unknown error"); \
    }


// Registration function for RichTextEffect
void register_RichTextEffect_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "RichTextEffect", class_info);
}
