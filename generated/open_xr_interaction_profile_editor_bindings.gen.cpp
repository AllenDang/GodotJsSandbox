...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "OpenXRInteractionProfileEditor: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "OpenXRInteractionProfileEditor: unknown error"); \
    }


// Registration function for OpenXRInteractionProfileEditor
void register_OpenXRInteractionProfileEditor_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "OpenXRInteractionProfileEditorBase"));
    ...
    JS_SetPropertyStr(ctx, classes, "OpenXRInteractionProfileEditor", class_info);
}
