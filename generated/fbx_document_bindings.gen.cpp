...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "FBXDocument: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "FBXDocument: unknown error"); \
    }


// Registration function for FBXDocument
void register_FBXDocument_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "GLTFDocument"));
    ...
    JS_SetPropertyStr(ctx, classes, "FBXDocument", class_info);
}
