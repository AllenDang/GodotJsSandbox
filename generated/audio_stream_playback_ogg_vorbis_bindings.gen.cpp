...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioStreamPlaybackOggVorbis: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioStreamPlaybackOggVorbis: unknown error"); \
    }


// Registration function for AudioStreamPlaybackOggVorbis
void register_AudioStreamPlaybackOggVorbis_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioStreamPlaybackOggVorbis", class_info);
}
