...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "CSGCombiner3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "CSGCombiner3D: unknown error"); \
    }


// Registration function for CSGCombiner3D
void register_CSGCombiner3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "CSGCombiner3D", class_info);
}
