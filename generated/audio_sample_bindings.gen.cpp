...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioSample: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioSample: unknown error"); \
    }


// Registration function for AudioSample
void register_AudioSample_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioSample", class_info);
}
