...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Node3DGizmo: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Node3DGizmo: unknown error"); \
    }


// Registration function for Node3DGizmo
void register_Node3DGizmo_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Node3DGizmo", class_info);
}
