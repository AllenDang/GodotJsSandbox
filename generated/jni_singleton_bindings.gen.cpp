...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "JNISingleton: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "JNISingleton: unknown error"); \
    }


// Registration function for JNISingleton
void register_JNISingleton_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "JNISingleton", class_info);
}
