...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "CheckBox: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "CheckBox: unknown error"); \
    }


// Registration function for CheckBox
void register_CheckBox_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "CheckBox", class_info);
}
