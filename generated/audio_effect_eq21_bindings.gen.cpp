...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectEQ21: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectEQ21: unknown error"); \
    }


// Registration function for AudioEffectEQ21
void register_AudioEffectEQ21_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "AudioEffectEQ"));
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectEQ21", class_info);
}
