...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "RenderSceneBuffersExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "RenderSceneBuffersExtension: unknown error"); \
    }


// Registration function for RenderSceneBuffersExtension
void register_RenderSceneBuffersExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "RenderSceneBuffersExtension", class_info);
}
