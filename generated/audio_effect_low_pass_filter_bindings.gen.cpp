...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectLowPassFilter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectLowPassFilter: unknown error"); \
    }


// Registration function for AudioEffectLowPassFilter
void register_AudioEffectLowPassFilter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectLowPassFilter", class_info);
}
