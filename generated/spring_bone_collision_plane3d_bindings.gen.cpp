...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "SpringBoneCollisionPlane3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "SpringBoneCollisionPlane3D: unknown error"); \
    }


// Registration function for SpringBoneCollisionPlane3D
void register_SpringBoneCollisionPlane3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "SpringBoneCollisionPlane3D", class_info);
}
