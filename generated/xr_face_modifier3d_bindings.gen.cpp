...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "XRFaceModifier3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "XRFaceModifier3D: unknown error"); \
    }

// Property getter: XRFaceModifier3D::target
static JSValue js_XRFaceModifier3D_get_target(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target getter: missing handle");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target getter: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target getter: invalid object");
    }

    XRFaceModifier3D* typed_obj = Object::cast_to<XRFaceModifier3D>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target getter: wrong type");
    }

    NodePath value = typed_obj->get_target();
    return JS_NewString(ctx, String(value).utf8().get_data());
}

// Property setter: XRFaceModifier3D::target
static JSValue js_XRFaceModifier3D_set_target(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 2) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target setter: missing arguments");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target setter: invalid handle");
    }
    ...
    if (limiter && !limiter->check_api_rate_limit(ApiCategory::WRITE)) {
        return JS_ThrowInternalError(ctx, "XRFaceModifier3D.target setter: write rate limit exceeded");
    }

    Object* obj = qjs_ctx->get_object_registry()->get_object(handle);
    if (!obj) {
        return JS_ThrowTypeError(ctx, "XRFaceModifier3D.target setter: invalid object");
    }
...
// Registration function for XRFaceModifier3D
void register_XRFaceModifier3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
...

    {
        ...
        JS_SetPropertyStr(ctx, prop_obj, "get",
            JS_NewCFunction(ctx, js_XRFaceModifier3D_get_target, "get_target", 1));
        JS_SetPropertyStr(ctx, prop_obj, "set",
            JS_NewCFunction(ctx, js_XRFaceModifier3D_set_target, "set_target", 2));
    ...
    JS_SetPropertyStr(ctx, classes, "XRFaceModifier3D", class_info);
}
