...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VSeparator: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VSeparator: unknown error"); \
    }


// Registration function for VSeparator
void register_VSeparator_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "VSeparator", class_info);
}
