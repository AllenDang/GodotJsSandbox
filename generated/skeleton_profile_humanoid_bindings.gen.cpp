...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "SkeletonProfileHumanoid: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "SkeletonProfileHumanoid: unknown error"); \
    }


// Registration function for SkeletonProfileHumanoid
void register_SkeletonProfileHumanoid_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "SkeletonProfile"));
    ...
    JS_SetPropertyStr(ctx, classes, "SkeletonProfileHumanoid", class_info);
}
