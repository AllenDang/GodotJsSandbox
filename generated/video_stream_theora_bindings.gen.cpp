...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VideoStreamTheora: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VideoStreamTheora: unknown error"); \
    }


// Registration function for VideoStreamTheora
void register_VideoStreamTheora_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "VideoStream"));
    ...
    JS_SetPropertyStr(ctx, classes, "VideoStreamTheora", class_info);
}
