...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PhysicsDirectSpaceState3DExtension: %s", e.what()); \
    } catch (...) { \
...
void register_PhysicsDirectSpaceState3DExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "PhysicsDirectSpaceState3D"));
    ...
    JS_SetPropertyStr(ctx, classes, "PhysicsDirectSpaceState3DExtension", class_info);
}
