...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "WebRTCPeerConnectionExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "WebRTCPeerConnectionExtension: unknown error"); \
    }


// Registration function for WebRTCPeerConnectionExtension
void register_WebRTCPeerConnectionExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "WebRTCPeerConnection"));
    ...
    JS_SetPropertyStr(ctx, classes, "WebRTCPeerConnectionExtension", class_info);
}
