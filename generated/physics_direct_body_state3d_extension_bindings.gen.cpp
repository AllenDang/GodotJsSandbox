...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "PhysicsDirectBodyState3DExtension: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "PhysicsDirectBodyState3DExtension: unknown error"); \
    }


// Registration function for PhysicsDirectBodyState3DExtension
void register_PhysicsDirectBodyState3DExtension_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, class_info, "parent", JS_NewString(ctx, "PhysicsDirectBodyState3D"));
    ...
    JS_SetPropertyStr(ctx, classes, "PhysicsDirectBodyState3DExtension", class_info);
}
