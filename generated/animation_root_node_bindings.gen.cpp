...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AnimationRootNode: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AnimationRootNode: unknown error"); \
    }


// Registration function for AnimationRootNode
void register_AnimationRootNode_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AnimationRootNode", class_info);
}
