...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectBandLimitFilter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectBandLimitFilter: unknown error"); \
    }


// Registration function for AudioEffectBandLimitFilter
void register_AudioEffectBandLimitFilter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectBandLimitFilter", class_info);
}
