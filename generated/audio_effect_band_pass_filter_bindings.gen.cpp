...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "AudioEffectBandPassFilter: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "AudioEffectBandPassFilter: unknown error"); \
    }


// Registration function for AudioEffectBandPassFilter
void register_AudioEffectBandPassFilter_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "AudioEffectBandPassFilter", class_info);
}
