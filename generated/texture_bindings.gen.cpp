...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Texture: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Texture: unknown error"); \
    }


// Registration function for Texture
void register_Texture_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Texture", class_info);
}
