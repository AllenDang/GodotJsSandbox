...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "Occluder3D: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "Occluder3D: unknown error"); \
    }

// Method: Occluder3D::get_vertices
static JSValue js_Occluder3D_get_vertices(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    if (argc < 1) {
        return JS_ThrowTypeError(ctx, "Occluder3D.get_vertices: missing handle argument");
    }

    int64_t handle;
    if (JS_ToInt64(ctx, &handle, argv[0]) < 0) {
        return JS_ThrowTypeError(ctx, "Occluder3D.get_vertices: invalid handle");
    }
    ...
    if (!obj) {
        return JS_ThrowTypeError(ctx, "Occluder3D.get_vertices: invalid or freed object");
    }

    Occluder3D* typed_obj = Object::cast_to<Occluder3D>(obj);
    if (!typed_obj) {
        return JS_ThrowTypeError(ctx, "Occluder3D.get_vertices: object is not a Occluder3D");
    }

    PackedVector3Array result = typed_obj->get_vertices();
...
void register_Occluder3D_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "Occluder3D", class_info);
}
