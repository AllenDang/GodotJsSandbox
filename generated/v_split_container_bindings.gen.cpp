...
#define SAFE_METHOD_CALL(code) \
    try { \
        ...
        return JS_ThrowInternalError(ctx, "VSplitContainer: %s", e.what()); \
    } catch (...) { \
        return JS_ThrowInternalError(ctx, "VSplitContainer: unknown error"); \
    }


// Registration function for VSplitContainer
void register_VSplitContainer_bindings(JSContext* ctx, JSValue global, JSValue classes) {
    ...
    JS_SetPropertyStr(ctx, classes, "VSplitContainer", class_info);
}
